...
PC2Merger::PC2Merger() : Node("pc2_multi_merger") {
	...
	point_cloud_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
		cloud_destination_topic_, rclcpp::SensorDataQoS());
}
...
void PC2Merger::pc2_topic_parser_() {
	...
	while (!tokens.empty()) {
		...
		for (const auto &topic_it: topics) {
			...
			if (std::find(topic_types.begin(), topic_types.end(), "sensor_msgs/msg/PointCloud2") != topic_types.end() &&
	...
	if ((tmp_input_topics.size() != input_topics_.size()) || !equal(tmp_input_topics.begin(), tmp_input_topics.end(),
	                                                                input_topics_.begin())) {
...
void PC2Merger::pc2_cb_(const sensor_msgs::msg::PointCloud2::SharedPtr &cloud, const std::string &topic) {
	...
	try {
		tf_buffer_->lookupTransform(cloud->header.frame_id, destination_frame_, cloud->header.stamp,
		                            rclcpp::Duration(1, 0));
